...
class ComplaintForm(ModelForm):
    ...
    office = forms.ModelChoiceField(queryset=offices.objects.all().exclude(office_name__icontains='Admin'),empty_label=' Choose Office',widget=forms.Select(attrs={'style':'width:180px'}))
    acc_person = forms.ModelChoiceField(queryset=office_staff.objects.all(),empty_label=' Choose Concerned person',widget=forms.Select(attrs={'style':'width:180px'}))
    complaint_type = forms.ModelChoiceField(queryset=comp_type.objects.all(),empty_label='Choose Complaint type',widget=forms.Select(attrs={'style':'width:180px'}))
    village = forms.ModelChoiceField(queryset=village.objects.all(),empty_label='Choose Village',widget=forms.Select(attrs={'style':'width:180px'}))
    taluka = forms.ModelChoiceField(queryset=taluka.objects.all(),empty_label=' Choose Taluka',widget=forms.Select(attrs={'style':'width:180px'}))
